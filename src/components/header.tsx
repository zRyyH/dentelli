    ...
    <>
      ...
      <header
        ...
        style={{
          ...
          boxShadow: scrolled ? "0 4px 28px rgba(0,0,0,0.07)" : "0 1px 0 rgba(0,0,0,0.06)",
        }}
      >
        {/* Logo row */}
        <div className="flex items-center justify-center px-4 pt-2 pb-1">
          <div className="cursor-pointer" onClick={() => router.push("/homepage")}>
            {images.logo && <img src={images.logo} alt="DentelliClub" className="h-7 object-contain" />}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 pb-2">
          <form onSubmit={handleSearch} className="flex-1">
            <div className="flex w-full items-center rounded-full border border-border/60 bg-muted/35 px-3 py-1.5 transition-all focus-within:border-primary/50 focus-within:bg-white">
          ...
          {/* Mobile cart — vai direto ao checkout */}
          <button onClick={() => router.push("/checkout")}
            className={`relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition-colors hover:bg-muted ${cartShake ? "animate-bounce" : ""}`}>
            <ShoppingCart className="h-[18px] w-[18px] text-muted-foreground/70" />
            {itemCount > 0 && (
          ...
          <button onClick={() => setMobileMenuOpen((v) => !v)}
            ...
            {mobileMenuOpen ? <X className="h-5 w-5 text-foreground/70" /> : <Menu className="h-5 w-5 text-foreground/70" />}
          </button>
        </div>

        {/* Mobile search dropdown — filho direto do header para não ficar preso */}
        {showSearch && searchQuery.trim() && (
          <div className="absolute left-0 right-0 top-full z-[70] overflow-hidden border-b border-border bg-card shadow-xl" onMouseDown={(e) => e.stopPropagation()}>
            {filteredProducts.length > 0 ? filteredProducts.map((product) => (
              <button key={product.id} onClick={() => { router.push(`/produto/${product.id}`); setSearchQuery(""); setShowSearch(false); }}
                className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-muted active:bg-muted">
                {getProductImageUrl(product) ? (
                  <img src={getProductImageUrl(product)} alt={product.nome} className="h-9 w-9 rounded-lg object-cover" />
            ...
            <button onClick={handleSearch as React.MouseEventHandler}
              className="w-full border-t border-border px-4 py-3 text-sm font-semibold text-primary hover:bg-muted active:bg-muted">
      ...
      {/* ── DESKTOP HEADER ── */}
      <header
        className="sticky top-0 z-50 hidden md:flex h-[62px] items-center px-6 transition-all duration-300"
        style={{
          background: scrolled ? "rgba(255,255,255,0.92)" : "white",
          backdropFilter: scrolled ? "blur(14px)" : "none",
      ...
      >
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4">
          ...
          {/* Search — desktop */}
          <div className="relative flex flex-1 justify-center" ref={searchRef}>
            ...
            {showSearch && searchQuery.trim() && (
              <div className="absolute left-0 right-0 top-full z-[70] mt-2 overflow-hidden rounded-xl border border-border bg-card shadow-xl" onMouseDown={(e) => e.stopPropagation()}>
                {filteredProducts.length > 0 ? filteredProducts.map((product) => (
                  <button key={product.id} onClick={() => { router.push(`/produto/${product.id}`); setSearchQuery(""); setShowSearch(false); }}
          ...
          <div className="flex shrink-0 items-center gap-0.5">
            {[
              ...
              <button key={path} onClick={() => router.push(path)}
                className={`group relative flex flex-col items-center gap-0.5 rounded-xl px-3 py-1.5 transition-all duration-200 ${pathname === path ? "bg-primary/10" : "hover:bg-primary/8"}`}>
            ...
            <Sheet open={cartOpen} onOpenChange={setCartOpen}>
              <SheetTrigger className={`group relative flex flex-col items-center gap-0.5 rounded-xl px-3 py-1.5 transition-all ${cartOpen ? "bg-primary/10" : "hover:bg-primary/8"} ${cartShake ? "animate-bounce" : ""}`}>
      ...
      {/* Desktop nav */}
      <nav className="hidden md:block w-full border-b border-border/50 bg-white">
      ...
      {/* Mobile nav drawer */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-[39] md:hidden" onClick={() => setMobileMenuOpen(false)}>
          <div className="absolute inset-0 bg-black/30" />
          <nav
            className="absolute left-0 right-0 top-[88px] bg-white shadow-xl border-b border-border"
            ...
            <div className="flex flex-col divide-y divide-border/50">
              {NAV_ITEMS.map((item) => {
                ...
                if (item.label === "CADASTRAR") {
                  ...
                  return (
                    <div key="cadastrar">
                      <p className="px-5 py-3 text-[11px] font-bold uppercase tracking-widest text-muted-foreground/50">Cadastrar</p>
                      {CADASTRAR_OPTIONS.map((opt) => (
                        <button key={opt.path} onClick={() => { router.push(opt.path); setMobileMenuOpen(false); }}
                          className="flex w-full items-center px-8 py-3 text-sm font-medium text-foreground/70 hover:bg-primary/5 hover:text-primary transition-colors">
                          {opt.label}
                        </button>
                      ))}
                    </div>
                ...
                return (
                  <button key={item.label} onClick={() => { item.path && router.push(item.path); setMobileMenuOpen(false); }}
                    className={`flex w-full items-center px-5 py-4 text-sm font-semibold tracking-wide transition-colors ${isActive ? "text-primary bg-primary/5" : "text-foreground/70 hover:bg-primary/5 hover:text-primary"}`}>
                    {item.label}
                  </button>
                );
              })}
              ...
              <div className="flex flex-col divide-y divide-border/50 border-t-2 border-border">
                {[
                  { icon: User, label: "Minha Conta", path: "/my_account" },
                  { icon: Heart, label: "Lista de Desejos", path: "/wishlist" },
                ].map(({ icon: Icon, label, path }) => (
                  <button key={path} onClick={() => { router.push(path); setMobileMenuOpen(false); }}
                    className={`flex w-full items-center gap-3 px-5 py-4 text-sm font-semibold transition-colors ${pathname === path ? "text-primary bg-primary/5" : "text-foreground/70 hover:bg-primary/5 hover:text-primary"}`}>
